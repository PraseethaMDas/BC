...
function populateDetails(item) {
    ...
    window.pm.methods.parts(item).call({ from: window.accounts[0] }, function (error, part_info) {
        if (error)
            ...
            document.getElementById("details-expiry-date").textContent = part_info["expiry_date"]
            document.getElementById("details-price").textContent = part_info["product_price"]
...
async function init_web3() {
    ...
    window.pm.options.address = '0x6fc45c0006bE5f2e786947aC45C536Ee3A14b55A'
    ...
    window.co.options.address = "0x99307e823A7e7eacdB2264e857fB0246fDfa0aCA"
}
...
function carPartListsManager(e) {
    //toggleActive(e)
    //clearActiveExcept(e)

    if (e.classList.contains("active")) {
        getOwnerHistoryFromEvents('TransferPartOwnership', e.textContent).then((owners) => {
            console.log(owners)
